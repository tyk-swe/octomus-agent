...
and `--export-run` examples on a synthetic database. `tests/systemd.py` requires root on a
disposable systemd VM and exercises the unit's write restrictions and child cleanup.
