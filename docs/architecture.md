...
Each cycle records the remote default-branch revision, open prefixed PRs, their heads and accumulated scope, maintenance targets, and task history. For owned PRs it also records feedback: the latest review state per reviewer, check-run status on the head commit with failing check names, GitHub's mergeability, and the most recent review and issue comments (at most 30, each truncated to 2000 characters and redacted, about 24 KB per PR). Owned PRs with requested changes, failing checks or conflicts become feedback targets; discovery is told to prioritize proposals that address that recorded feedback on the PR branch, and consolidation prefers such work over new default-branch work of similar value. Comment text is evidence, never instruction. The orchestrator inspects the repository and PR diffs. Each discovery and proposal-review role has a separate clone and Codex thread. Planning sessions are instructed to inspect rather than mutate, and their worktree/HEAD must remain unchanged.
